...
public class EnemyBirdScript : MonoBehaviour
{
    ...
    void Update()
    {
        if (isDead || waypoints.Length == 0) return; // Vogel bewegt sich nicht, wenn er tot ist oder keine Wegpunkte existieren

        // Bewegung zum aktuellen Wegpunkt
        Transform targetWaypoint = waypoints[currentWaypointIndex];
        Vector2 direction = (targetWaypoint.position - transform.position).normalized;
        rb.velocity = direction * speed;

        // Überprüfen, ob der Wegpunkt erreicht wurde
        if (Vector2.Distance(transform.position, targetWaypoint.position) < waypointTolerance)
        {
            currentWaypointIndex = (currentWaypointIndex + 1) % waypoints.Length; // Zum nächsten Wegpunkt wechseln
        }
    }
